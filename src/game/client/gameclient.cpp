...
void CGameClient::OnConsoleInit()
{
	...
	Console()->Register("toggle_system_gui", "", CFGFLAG_CLIENT, ConToggleSystemGui, this, "Toggle System Client GUI");
...
int CGameClient::OnSnapInput(int *pData, bool Dummy, bool Force)
{
	...
	{
		const bool BypassTickGate = m_WibeVisuals.m_AutoPseudoEnabled;
		if(!BypassTickGate && m_DummyFire % 25 != 0)
		{
			m_DummyFire++;
			return 0;
		}
		...
		// Keep deep-fly compatible with Auto Triple / Auto Pseudo hook logic.
		// If these systems already prepared smart target/hook in dummy input, reuse it.
		if(m_WibeVisuals.m_AutoTripleEnabled || m_WibeVisuals.m_AutoPseudoEnabled)
		{
			if(m_DummyInput.m_TargetX != 0 || m_DummyInput.m_TargetY != 0)
			{
		...
		// In auto pseudo mode, fire is fully controlled by pseudo logic (no extra tick/range gate here).
		if(m_WibeVisuals.m_AutoPseudoEnabled)
...
void CGameClient::ConToggleSystemGui(IConsole::IResult *pResult, void *pUserData)
{
	CGameClient *pClient = static_cast<CGameClient *>(pUserData);
	(void)pResult;

	if(pClient->m_Menus.IsActive())
	{
		pClient->m_Menus.SetActive(false);
		return;
	}

	pClient->m_Menus.SetActive(true);
	pClient->m_Menus.SetShowStart(false);
	pClient->m_Menus.SetMenuPage(CMenus::PAGE_SETTINGS);
	g_Config.m_UiSettingsPage = CMenus::SETTINGS_SYSTEM;
}
