...
int CControls::SnapInput(int *pData)
{
	...
	else
	{
		...
		// System Client misc input features: Auto Fire only.
		{
			// Auto Fire - may modify m_Fire flag when enabled.
			int Fire = m_aInputData[g_Config.m_ClDummy].m_Fire;
			GameClient()->m_WibeVisuals.ApplyAutoFire(&Fire);
			m_aInputData[g_Config.m_ClDummy].m_Fire = Fire;
		}
		...
		if(g_Config.m_ClDummyCopyMoves)
		{
			...
			if(!GameClient()->m_Snap.m_SpecInfo.m_Active || GameClient()->m_Snap.m_SpecInfo.m_SpectatorId < 0)
			{
				...
				// Auto Pseudo: smart hammer/hook flow for deep-fly behavior.
				GameClient()->m_WibeVisuals.ApplyAutoPseudo(&m_aInputData[g_Config.m_ClDummy], pDummyInput);

				// Auto Triple: allow it to coexist with pseudo (triple controls hook/jerk behavior).
				GameClient()->m_WibeVisuals.ApplyAutoTriple(&m_aInputData[g_Config.m_ClDummy], pDummyInput);
			}

			m_aInputData[!g_Config.m_ClDummy] = *pDummyInput;
		}
		else if(GameClient()->m_WibeVisuals.m_AutoPseudoEnabled && !g_Config.m_ClDummyControl)
		{
			// Allow auto pseudo to work even when copy-moves is disabled.
			CNetObj_PlayerInput *pDummyInput = &GameClient()->m_DummyInput;
			GameClient()->m_WibeVisuals.ApplyAutoPseudo(&m_aInputData[g_Config.m_ClDummy], pDummyInput);
			m_aInputData[!g_Config.m_ClDummy] = *pDummyInput;
		}
...
void CControls::OnRender()
{
	...
	bool AutoPseudoBindPressed = false;
	// Toggle-bind for Auto Pseudo state.
	if(g_Config.m_WibeAutoPseudoBind[0] != '\0')
	{
		const int Key = Input()->FindKeyByName(g_Config.m_WibeAutoPseudoBind);
		if(Key >= 0 && Input()->KeyPress(Key))
		{
			GameClient()->m_WibeVisuals.m_AutoPseudoEnabled ^= 1;
			AutoPseudoBindPressed = true;
		}
	}

	// Toggle bind for Auto Triple.
	// If both features accidentally use same key, pseudo has priority this frame.
	if(g_Config.m_WibeAutoTripleBind[0] != '\0')
	{
		const int Key = Input()->FindKeyByName(g_Config.m_WibeAutoTripleBind);
		if(Key >= 0 && Input()->KeyPress(Key) && !AutoPseudoBindPressed)
			GameClient()->m_WibeVisuals.m_AutoTripleEnabled ^= 1;
	}
}
