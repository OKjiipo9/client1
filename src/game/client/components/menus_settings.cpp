...
void CMenus::RenderSettings(CUIRect MainView)
{
	...
	else if(g_Config.m_UiSettingsPage == SETTINGS_SYSTEM)
	{
		GameClient()->m_MenuBackground.ChangePosition(CMenuBackground::POS_SETTINGS_GRAPHICS);
		RenderSettingsSystem(MainView);
	}
	else
	{
...
void CMenus::RenderSettingsSystem(CUIRect MainView)
{
	CUIRect Label, Button, Slider;

	MainView.HSplitTop(30.0f, &Label, &MainView);
	Ui()->DoLabel(&Label, "System Client", 20.0f, TEXTALIGN_ML);
	MainView.HSplitTop(10.0f, nullptr, &MainView);
	...
	static int s_ActiveTab = SYSTEMTAB_GENERAL;
	static CButtonContainer s_aTabButtons[NUM_SYSTEM_TABS];

	const char *apTabNames[NUM_SYSTEM_TABS] = {
		"General",
		"General2",
		"Visuals",
		"Dummy",
	...
	// Sub-tab bar
	CUIRect TabBar;
	MainView.HSplitTop(30.0f, &TabBar, &MainView);
	const float TabWidth = TabBar.w / (float)NUM_SYSTEM_TABS;
	CUIRect Tabs = TabBar;
	CUIRect TabRect;
	for(int i = 0; i < NUM_SYSTEM_TABS; i++)
	{
		Tabs.VSplitLeft(TabWidth, &TabRect, &Tabs);
		const int Corners = i == SYSTEMTAB_GENERAL ? IGraphics::CORNER_L : (i == NUM_SYSTEM_TABS - 1 ? IGraphics::CORNER_R : IGraphics::CORNER_NONE);
		if(DoButton_MenuTab(&s_aTabButtons[i], apTabNames[i], s_ActiveTab == i, &TabRect, Corners, nullptr, nullptr, nullptr, nullptr, 4.0f))
			s_ActiveTab = i;
	}

	MainView.HSplitTop(10.0f, nullptr, &MainView);

	CSystemVisuals *pVisuals = &GameClient()->m_SystemVisuals;

	// Scrollable content so long menus are reachable.
	static CScrollRegion s_SystemScrollRegion;
	static int s_LastTab = -1;
	vec2 ScrollOffset(0.0f, 0.0f);
	CScrollRegionParams ScrollParams;
	ScrollParams.m_ScrollUnit = 20.0f;
	ScrollParams.m_Flags = CScrollRegionParams::FLAG_CONTENT_STATIC_WIDTH;
	if(s_LastTab != s_ActiveTab)
	{
		s_SystemScrollRegion.Reset();
		s_LastTab = s_ActiveTab;
	}
	s_SystemScrollRegion.Begin(&MainView, &ScrollOffset, &ScrollParams);
	MainView.y += ScrollOffset.y;

	// ============== GENERAL ==============
	if(s_ActiveTab == SYSTEMTAB_GENERAL)
	{
		CWibeVisuals *pWibeVisuals = &GameClient()->m_WibeVisuals;
		static int s_WaitingBind = 0; // 1=auto pseudo, 2=auto triple
		MainView.HSplitTop(22.0f, &Label, &MainView);
		s_SystemScrollRegion.AddRect(Label);
		Ui()->DoLabel(&Label, "General", 16.0f, TEXTALIGN_ML);

		MainView.HSplitTop(6.0f, nullptr, &MainView);
		MainView.HSplitTop(18.0f, &Label, &MainView);
		s_SystemScrollRegion.AddRect(Label);
		Ui()->DoLabel(&Label, "fly mode", 12.0f, TEXTALIGN_ML);

		CUIRect Row, LeftCol, RightCol, BindLabel, BindButton;
		MainView.HSplitTop(22.0f, &Row, &MainView);
		s_SystemScrollRegion.AddRect(Row);
		Row.VSplitMid(&LeftCol, &RightCol, 16.0f);
		if(DoButton_CheckBox(&pWibeVisuals->m_AutoPseudoEnabled, "auto pseudo", pWibeVisuals->m_AutoPseudoEnabled, &LeftCol))
			pWibeVisuals->m_AutoPseudoEnabled ^= 1;
		RightCol.VSplitLeft(95.0f, &BindLabel, &BindButton);
		Ui()->DoLabel(&BindLabel, "auto pseudo", 11.0f, TEXTALIGN_ML);
		static CButtonContainer s_AutoPseudoBindButton;
		char aPseudoBindText[32];
		if(s_WaitingBind == 1)
			str_copy(aPseudoBindText, "Press key...");
		else if(g_Config.m_WibeAutoPseudoBind[0])
			str_copy(aPseudoBindText, g_Config.m_WibeAutoPseudoBind, sizeof(aPseudoBindText));
		else
			str_copy(aPseudoBindText, "[Bind]", sizeof(aPseudoBindText));
		if(DoButton_Menu(&s_AutoPseudoBindButton, aPseudoBindText, 0, &BindButton))
			s_WaitingBind = 1;

		MainView.HSplitTop(22.0f, &Row, &MainView);
		s_SystemScrollRegion.AddRect(Row);
		Row.VSplitMid(&LeftCol, &RightCol, 16.0f);
		if(DoButton_CheckBox(&pWibeVisuals->m_AutoTripleEnabled, "auto triple", pWibeVisuals->m_AutoTripleEnabled, &LeftCol))
			pWibeVisuals->m_AutoTripleEnabled ^= 1;
		RightCol.VSplitLeft(95.0f, &BindLabel, &BindButton);
		Ui()->DoLabel(&BindLabel, "auto triple", 11.0f, TEXTALIGN_ML);
		static CButtonContainer s_AutoTripleBindButton;
		char aTripleBindText[32];
		if(s_WaitingBind == 2)
			str_copy(aTripleBindText, "Press key...");
		else if(g_Config.m_WibeAutoTripleBind[0])
			str_copy(aTripleBindText, g_Config.m_WibeAutoTripleBind, sizeof(aTripleBindText));
		else
			str_copy(aTripleBindText, "[Bind]", sizeof(aTripleBindText));
		if(DoButton_Menu(&s_AutoTripleBindButton, aTripleBindText, 0, &BindButton))
			s_WaitingBind = 2;

		MainView.HSplitTop(18.0f, &Slider, &MainView);
		s_SystemScrollRegion.AddRect(Slider);
		Ui()->DoScrollbarOption(&pWibeVisuals->m_AutoTriplePreset, &pWibeVisuals->m_AutoTriplePreset, &Slider, "auto triple mode (1 custom, 2 fun)", 1, 2, &CUi::ms_LinearScrollbarScale);

		MainView.HSplitTop(16.0f, &Label, &MainView);
		s_SystemScrollRegion.AddRect(Label);
		Ui()->DoLabel(&Label, "triple hook now follows main hook", 10.0f, TEXTALIGN_ML);

		if(pWibeVisuals->m_AutoTriplePreset == 1)
		{
			MainView.HSplitTop(18.0f, &Slider, &MainView);
			s_SystemScrollRegion.AddRect(Slider);
			Ui()->DoScrollbarOption(&pWibeVisuals->m_AutoTripleHookRadiusDeg, &pWibeVisuals->m_AutoTripleHookRadiusDeg, &Slider, "hook radius (deg)", 30, 360, &CUi::ms_LinearScrollbarScale);
		}
		...
		if(s_WaitingBind != 0)
		{
			for(int Key = KEY_FIRST; Key < KEY_LAST; Key++)
			{
				...
				if(s_WaitingBind == 1)
				{
					str_copy(g_Config.m_WibeAutoPseudoBind, pKeyName, sizeof(g_Config.m_WibeAutoPseudoBind));
					// Keep binds conflict-free: one key controls one feature.
					if(!str_comp(g_Config.m_WibeAutoPseudoBind, g_Config.m_WibeAutoTripleBind))
						g_Config.m_WibeAutoTripleBind[0] = '\0';
				}
				else
				{
					str_copy(g_Config.m_WibeAutoTripleBind, pKeyName, sizeof(g_Config.m_WibeAutoTripleBind));
					// Keep binds conflict-free: one key controls one feature.
					if(!str_comp(g_Config.m_WibeAutoTripleBind, g_Config.m_WibeAutoPseudoBind))
						g_Config.m_WibeAutoPseudoBind[0] = '\0';
				}
				s_WaitingBind = 0;
				break;
			}
	...
	// ============== GENERAL2 ==============
	else if(s_ActiveTab == SYSTEMTAB_GENERAL2)
	{
		MainView.HSplitTop(22.0f, &Label, &MainView);
		s_SystemScrollRegion.AddRect(Label);
		Ui()->DoLabel(&Label, "General2", 16.0f, TEXTALIGN_ML);

		MainView.HSplitTop(20.0f, &Label, &MainView);
		s_SystemScrollRegion.AddRect(Label);
		Ui()->DoLabel(&Label, "TAB_MISC options moved to `Misc` tab.", 12.0f, TEXTALIGN_ML);

		MainView.HSplitTop(10.0f, nullptr, &MainView);
	}

	// ============== VISUALS (weapon FX) ==============
	else if(s_ActiveTab == SYSTEMTAB_VISUALS)
	{
		MainView.HSplitTop(22.0f, &Label, &MainView);
		s_SystemScrollRegion.AddRect(Label);
		Ui()->DoLabel(&Label, "Visuals", 16.0f, TEXTALIGN_ML);

		MainView.HSplitTop(8.0f, nullptr, &MainView);

		// ============== Visual base (all non-weapon visuals) ==============
		MainView.HSplitTop(18.0f, &Button, &MainView);
		s_SystemScrollRegion.AddRect(Button);
		if(DoButton_CheckBox(&pVisuals->m_WaterMarkEnabled, "WaterMark - Client branding", pVisuals->m_WaterMarkEnabled, &Button))
			pVisuals->m_WaterMarkEnabled ^= 1;

		// Shared HUD theme for WaterMark / FunctionList / SpeedTab
		MainView.HSplitTop(1.0f, nullptr, &MainView);
		CUIRect HudThemeDropDown;
		MainView.HSplitTop(18.0f, &HudThemeDropDown, &MainView);
		const char *apHudThemeStyles[] = {"Glass", "Minimal", "Neon", "Aurora", "Cyber", "Sunset", "Midnight", "Pastel"};
		static const int s_NumHudThemeStyles = std::size(apHudThemeStyles);
		static CUi::SDropDownState s_HudThemeDropDownState;
		s_HudThemeDropDownState.m_SelectionPopupContext.m_pScrollRegion = &s_SystemScrollRegion;
		pVisuals->m_HudThemeStyle = Ui()->DoDropDown(&HudThemeDropDown, pVisuals->m_HudThemeStyle, apHudThemeStyles, s_NumHudThemeStyles, s_HudThemeDropDownState);

		MainView.HSplitTop(4.0f, nullptr, &MainView);
		CUIRect Row, LeftCol, RightCol;

		// Compact two-column toggles
		MainView.HSplitTop(18.0f, &Row, &MainView);
		s_SystemScrollRegion.AddRect(Row);
		Row.VSplitMid(&LeftCol, &RightCol, 8.0f);
		if(DoButton_CheckBox(&pVisuals->m_FunctionListEnabled, "Function List", pVisuals->m_FunctionListEnabled, &LeftCol))
			pVisuals->m_FunctionListEnabled ^= 1;
		if(DoButton_CheckBox(&pVisuals->m_SpeedTabEnabled, "Speed Tab", pVisuals->m_SpeedTabEnabled, &RightCol))
			pVisuals->m_SpeedTabEnabled ^= 1;

		MainView.HSplitTop(18.0f, &Row, &MainView);
		s_SystemScrollRegion.AddRect(Row);
		Row.VSplitMid(&LeftCol, &RightCol, 8.0f);
		if(DoButton_CheckBox(&pVisuals->m_RainbowMeEnabled, "Rainbow Me", pVisuals->m_RainbowMeEnabled, &LeftCol))
			pVisuals->m_RainbowMeEnabled ^= 1;
		if(DoButton_CheckBox(&pVisuals->m_PredictionEnabled, "Prediction", pVisuals->m_PredictionEnabled, &RightCol))
			pVisuals->m_PredictionEnabled ^= 1;

		if(pVisuals->m_FunctionListEnabled)
		{
			MainView.HSplitTop(18.0f, &Button, &MainView);
			s_SystemScrollRegion.AddRect(Button);
			if(DoButton_CheckBox(&pVisuals->m_FunctionListRainbow, "Function List: Rainbow Colors", pVisuals->m_FunctionListRainbow, &Button))
				pVisuals->m_FunctionListRainbow ^= 1;
		}

		if(pVisuals->m_PredictionEnabled)
		{
			MainView.HSplitTop(18.0f, &Slider, &MainView);
			s_SystemScrollRegion.AddRect(Slider);
			char aBuf[64];
			str_format(aBuf, sizeof(aBuf), "Distance: %d tiles", pVisuals->m_PredictionDistance);
		...
		MainView.HSplitTop(22.0f, &Label, &MainView);
		s_SystemScrollRegion.AddRect(Label);
		Ui()->DoLabel(&Label, Localize("Hammer FX Lab"), 16.0f, TEXTALIGN_ML);

		MainView.HSplitTop(14.0f, &Label, &MainView);
		s_SystemScrollRegion.AddRect(Label);
		Ui()->DoLabel(&Label, "Build your own impact: style + intensity + trail.", 11.0f, TEXTALIGN_ML);

		CUIRect HammerStyleDropDown;
		MainView.HSplitTop(18.0f, &HammerStyleDropDown, &MainView);
		s_SystemScrollRegion.AddRect(HammerStyleDropDown);
		static const char *s_apHammerFxStyles[] = {"Impact", "Storm", "Nova", "Void", "Frost", "Inferno"};
		static CUi::SDropDownState s_HammerFxStyleDropDownState;
		s_HammerFxStyleDropDownState.m_SelectionPopupContext.m_pScrollRegion = &s_SystemScrollRegion;
		g_Config.m_ClSysHammerHitFxStyle = Ui()->DoDropDown(&HammerStyleDropDown, g_Config.m_ClSysHammerHitFxStyle, s_apHammerFxStyles, std::size(s_apHammerFxStyles), s_HammerFxStyleDropDownState);

		// Simple toggles: FX layer + optional trail.
		// Trail requires FX (if you enable trail, FX will auto-enable; if FX is off, trail is forced off).
		MainView.HSplitTop(18.0f, &Button, &MainView);
		s_SystemScrollRegion.AddRect(Button);
		if(DoButton_CheckBox(&g_Config.m_ClSysHammerHitFx, Localize("Hammer hit FX"), g_Config.m_ClSysHammerHitFx, &Button))
		{
			g_Config.m_ClSysHammerHitFx ^= 1;
			if(!g_Config.m_ClSysHammerHitFx)
				g_Config.m_ClSysHammerHitTrail = 0;
		}

		MainView.HSplitTop(18.0f, &Button, &MainView);
		s_SystemScrollRegion.AddRect(Button);
		if(DoButton_CheckBox(&g_Config.m_ClSysHammerHitTrail, Localize("Hammer hit trail"), g_Config.m_ClSysHammerHitTrail, &Button))
		{
			g_Config.m_ClSysHammerHitTrail ^= 1;
			if(g_Config.m_ClSysHammerHitTrail)
				g_Config.m_ClSysHammerHitFx = 1;
		}

		// These affect the enabled Hammer FX layers.
		MainView.HSplitTop(34.0f, &Button, &MainView);
		s_SystemScrollRegion.AddRect(Button);
		Ui()->DoScrollbarOption(&g_Config.m_ClSysHammerHitFxIntensity, &g_Config.m_ClSysHammerHitFxIntensity, &Button, Localize("Hammer FX intensity"), 0, 200, &CUi::ms_LinearScrollbarScale, CUi::SCROLLBAR_OPTION_MULTILINE);

		MainView.HSplitTop(4.0f, nullptr, &MainView);

		CUIRect PresetsRow, PresetA, PresetB, PresetC;
		MainView.HSplitTop(20.0f, &PresetsRow, &MainView);
		s_SystemScrollRegion.AddRect(PresetsRow);
		PresetsRow.VSplitLeft((PresetsRow.w - 8.0f) / 3.0f, &PresetA, &PresetsRow);
		PresetsRow.VSplitLeft(4.0f, nullptr, &PresetsRow);
		PresetsRow.VSplitLeft((PresetsRow.w - 4.0f) / 2.0f, &PresetB, &PresetC);
		static CButtonContainer s_HammerPresetCompetitive;
		static CButtonContainer s_HammerPresetStylish;
		static CButtonContainer s_HammerPresetInsane;
		if(DoButton_Menu(&s_HammerPresetCompetitive, "Comp", 0, &PresetA))
		{
			g_Config.m_ClSysHammerHitFx = 1;
			g_Config.m_ClSysHammerHitTrail = 0;
			g_Config.m_ClSysHammerHitFxIntensity = 70;
			g_Config.m_ClSysHammerHitFxStyle = 0;
		}
		if(DoButton_Menu(&s_HammerPresetStylish, "Stylish", 0, &PresetB))
		{
			g_Config.m_ClSysHammerHitFx = 1;
			g_Config.m_ClSysHammerHitTrail = 1;
			g_Config.m_ClSysHammerHitFxIntensity = 120;
			g_Config.m_ClSysHammerHitFxStyle = 1;
		}
		if(DoButton_Menu(&s_HammerPresetInsane, "Insane", 0, &PresetC))
		{
			g_Config.m_ClSysHammerHitFx = 1;
			g_Config.m_ClSysHammerHitTrail = 1;
			g_Config.m_ClSysHammerHitFxIntensity = 180;
			g_Config.m_ClSysHammerHitFxStyle = 2;
		}

		CUIRect PresetsRow2, PresetD, PresetE;
		MainView.HSplitTop(4.0f, nullptr, &MainView);
		MainView.HSplitTop(20.0f, &PresetsRow2, &MainView);
		s_SystemScrollRegion.AddRect(PresetsRow2);
		PresetsRow2.VSplitMid(&PresetD, &PresetE, 8.0f);
		static CButtonContainer s_HammerPresetFrost;
		static CButtonContainer s_HammerPresetInferno;
		if(DoButton_Menu(&s_HammerPresetFrost, "Frost", 0, &PresetD))
		{
			g_Config.m_ClSysHammerHitFx = 1;
			g_Config.m_ClSysHammerHitTrail = 1;
			g_Config.m_ClSysHammerHitFxIntensity = 145;
			g_Config.m_ClSysHammerHitFxStyle = 4;
		}
		if(DoButton_Menu(&s_HammerPresetInferno, "Inferno", 0, &PresetE))
		{
			g_Config.m_ClSysHammerHitFx = 1;
			g_Config.m_ClSysHammerHitTrail = 1;
			g_Config.m_ClSysHammerHitFxIntensity = 200;
			g_Config.m_ClSysHammerHitFxStyle = 5;
		}
		...
		s_SystemScrollRegion.AddRect(Button);
		static CButtonContainer s_HammerFxColorResetId;
		DoLine_ColorPicker(&s_HammerFxColorResetId, 25.0f, 13.0f, 2.0f, &Button, Localize("Hammer FX color"), &g_Config.m_ClSysHammerHitFxColor, color_cast<ColorRGBA>(ColorHSLA(0x66CCFFFFU, true)), false, nullptr, true);

		// Bullet trail
		MainView.HSplitTop(10.0f, nullptr, &MainView);
		MainView.HSplitTop(22.0f, &Label, &MainView);
		s_SystemScrollRegion.AddRect(Label);
		Ui()->DoLabel(&Label, Localize("Gun / Bullet"), 16.0f, TEXTALIGN_ML);

		MainView.HSplitTop(18.0f, &Button, &MainView);
		s_SystemScrollRegion.AddRect(Button);
		if(DoButton_CheckBox(&g_Config.m_ClSysBulletTrailFx, Localize("Enhanced bullet trails"), g_Config.m_ClSysBulletTrailFx, &Button))
			g_Config.m_ClSysBulletTrailFx ^= 1;

		MainView.HSplitTop(34.0f, &Button, &MainView);
		s_SystemScrollRegion.AddRect(Button);
		if(g_Config.m_ClSysBulletTrailFx)
			Ui()->DoScrollbarOption(&g_Config.m_ClSysBulletTrailFxIntensity, &g_Config.m_ClSysBulletTrailFxIntensity, &Button, Localize("Bullet FX intensity"), 0, 200, &CUi::ms_LinearScrollbarScale, CUi::SCROLLBAR_OPTION_MULTILINE);

		MainView.HSplitTop(4.0f, nullptr, &MainView);
		MainView.HSplitTop(25.0f, &Button, &MainView);
		s_SystemScrollRegion.AddRect(Button);
		static CButtonContainer s_BulletFxColorResetId;
		DoLine_ColorPicker(&s_BulletFxColorResetId, 25.0f, 13.0f, 2.0f, &Button, Localize("Bullet FX color"), &g_Config.m_ClSysBulletTrailFxColor, color_cast<ColorRGBA>(ColorHSLA(0xFFFFFFFFU, true)), false, nullptr, true);

		// Air jump
		MainView.HSplitTop(10.0f, nullptr, &MainView);
		MainView.HSplitTop(22.0f, &Label, &MainView);
		s_SystemScrollRegion.AddRect(Label);
		Ui()->DoLabel(&Label, Localize("Movement"), 16.0f, TEXTALIGN_ML);

		MainView.HSplitTop(18.0f, &Button, &MainView);
		s_SystemScrollRegion.AddRect(Button);
		if(DoButton_CheckBox(&g_Config.m_ClSysAirJumpFx, Localize("Enhanced air jump effects"), g_Config.m_ClSysAirJumpFx, &Button))
			g_Config.m_ClSysAirJumpFx ^= 1;

		MainView.HSplitTop(34.0f, &Button, &MainView);
		s_SystemScrollRegion.AddRect(Button);
		if(g_Config.m_ClSysAirJumpFx)
			Ui()->DoScrollbarOption(&g_Config.m_ClSysAirJumpFxIntensity, &g_Config.m_ClSysAirJumpFxIntensity, &Button, Localize("Air jump FX intensity"), 0, 200, &CUi::ms_LinearScrollbarScale, CUi::SCROLLBAR_OPTION_MULTILINE);

		MainView.HSplitTop(4.0f, nullptr, &MainView);
		MainView.HSplitTop(25.0f, &Button, &MainView);
		s_SystemScrollRegion.AddRect(Button);
		static CButtonContainer s_AirJumpFxColorResetId;
		DoLine_ColorPicker(&s_AirJumpFxColorResetId, 25.0f, 13.0f, 2.0f, &Button, Localize("Air jump FX color"), &g_Config.m_ClSysAirJumpFxColor, color_cast<ColorRGBA>(ColorHSLA(0x66CCFFFFU, true)), false, nullptr, true);
	}

	// ============== TAS (legit) ==============
	else if(s_ActiveTab == SYSTEMTAB_TAS)
	{
		MainView.HSplitTop(22.0f, &Label, &MainView);
		s_SystemScrollRegion.AddRect(Label);
		...
		const bool DummyAllowed = Client()->DummyAllowed();
		const bool DummyConnecting = Client()->DummyConnecting();
		...
		if(DummyConnected)
		{
			...
			Ui()->DoLabel(&Label, "Status: Dummy connected", 13.0f, TEXTALIGN_ML);
		}
		else if(DummyConnecting)
		{
			TextRender()->TextColor(1.0f, 0.95f, 0.2f, 1.0f);
			Ui()->DoLabel(&Label, "Status: Connecting...", 13.0f, TEXTALIGN_ML);
		}
		else if(!DummyAllowed)
		{
			TextRender()->TextColor(1.0f, 0.35f, 0.35f, 1.0f);
			Ui()->DoLabel(&Label, "Status: Dummy is not allowed on this server", 13.0f, TEXTALIGN_ML);
		}
		else
		{
			TextRender()->TextColor(0.8f, 0.8f, 0.8f, 1.0f);
			Ui()->DoLabel(&Label, "Status: Not connected", 13.0f, TEXTALIGN_ML);
		}
		TextRender()->TextColor(1.0f, 1.0f, 1.0f, 1.0f);
		...
		MainView.HSplitTop(18.0f, &Label, &MainView);
		s_SystemScrollRegion.AddRect(Label);
		Ui()->DoLabel(&Label, "Dummy nickname", 12.0f, TEXTALIGN_ML);

		MainView.HSplitTop(6.0f, nullptr, &MainView);
		MainView.HSplitTop(28.0f, &Button, &MainView);
		s_SystemScrollRegion.AddRect(Button);
		static CLineInput s_DummyNickInput;
		s_DummyNickInput.SetBuffer(g_Config.m_ClDummyName, sizeof(g_Config.m_ClDummyName));
		s_DummyNickInput.SetEmptyText("Enter dummy nickname");
		Ui()->DoEditBox(&s_DummyNickInput, &Button, 12.0f);
		...
		if(DummyConnected)
		{
			if(DoButton_Menu(&s_DummyConnectButton, "Disconnect Dummy", 0, &Button))
				Client()->DummyDisconnect(nullptr);
		}
		else if(DummyAllowed && !DummyConnecting)
		{
			if(DoButton_Menu(&s_DummyConnectButton, "Connect Dummy", 0, &Button))
				Client()->DummyConnect();
		}
		else
		{
			Ui()->DoLabel(&Button, DummyConnecting ? "Connecting..." : "Dummy unavailable", 12.0f, TEXTALIGN_MC);
		}
		...
		if(DummyConnected)
		{
			MainView.HSplitTop(24.0f, &Button, &MainView);
			s_SystemScrollRegion.AddRect(Button);
			static CButtonContainer s_DummySwitchButton;
			char aSwitchText[64];
			str_format(aSwitchText, sizeof(aSwitchText), "Switch to %s", g_Config.m_ClDummy ? "Main" : "Dummy");
			if(DoButton_Menu(&s_DummySwitchButton, aSwitchText, 0, &Button))
		...
		MainView.HSplitTop(18.0f, &Label, &MainView);
		s_SystemScrollRegion.AddRect(Label);
		Ui()->DoLabel(&Label, "Extra dummy: launches another client instance", 11.0f, TEXTALIGN_ML);

		MainView.HSplitTop(6.0f, nullptr, &MainView);
		MainView.HSplitTop(24.0f, &Button, &MainView);
		s_SystemScrollRegion.AddRect(Button);
		static CButtonContainer s_ExtraDummyLaunchButton;
		static bool s_ExtraDummyLaunchTried = false;
		static bool s_ExtraDummyLaunchOk = false;
		if(DoButton_Menu(&s_ExtraDummyLaunchButton, "Launch extra client", 0, &Button))
		{
			char aClientBinaryPath[IO_MAX_PATH_LENGTH];
			Storage()->GetBinaryPath(PLAT_CLIENT_EXEC, aClientBinaryPath, sizeof(aClientBinaryPath));
			char aConnectCommand[256];
			const char *apArgs[1] = {nullptr};
			size_t NumArgs = 0;
			if(Client()->State() == IClient::STATE_ONLINE)
			{
				...
				{
					str_format(aConnectCommand, sizeof(aConnectCommand), "connect %s", CurrentServerInfo.m_aAddress);
					apArgs[0] = aConnectCommand;
					NumArgs = 1;
				}
			}
			const PROCESS Process = process_execute(aClientBinaryPath, EShellExecuteWindowState::FOREGROUND, apArgs, NumArgs);
			s_ExtraDummyLaunchTried = true;
			s_ExtraDummyLaunchOk = Process != INVALID_PROCESS;
		}

		if(s_ExtraDummyLaunchTried)
		{
			MainView.HSplitTop(6.0f, nullptr, &MainView);
			MainView.HSplitTop(16.0f, &Label, &MainView);
			s_SystemScrollRegion.AddRect(Label);
			if(s_ExtraDummyLaunchOk)
			{
				TextRender()->TextColor(0.2f, 1.0f, 0.2f, 1.0f);
				Ui()->DoLabel(&Label, "Extra client launched (auto-connects when possible).", 11.0f, TEXTALIGN_ML);
			}
			else
			{
				TextRender()->TextColor(1.0f, 0.35f, 0.35f, 1.0f);
	...
	else if(s_ActiveTab == SYSTEMTAB_MISC)
	{
		MainView.HSplitTop(22.0f, &Label, &MainView);
		s_SystemScrollRegion.AddRect(Label);
		Ui()->DoLabel(&Label, "Misc", 16.0f, TEXTALIGN_ML);

		MainView.HSplitTop(16.0f, &Label, &MainView);
		s_SystemScrollRegion.AddRect(Label);
		Ui()->DoLabel(&Label, "Compact misc controls", 12.0f, TEXTALIGN_ML);
		...
		MainView.HSplitTop(20.0f, &Button, &MainView);
		s_SystemScrollRegion.AddRect(Button);
		if(DoButton_CheckBox(&pWibeVisuals->m_ReactionSpamEnabled, "Reaction Spam - Rapidly spam emotes", pWibeVisuals->m_ReactionSpamEnabled, &Button))
			pWibeVisuals->m_ReactionSpamEnabled ^= 1;

		// Reaction Spam Mode selector
		if(pWibeVisuals->m_ReactionSpamEnabled)
		{
			MainView.HSplitTop(5.0f, nullptr, &MainView);
			MainView.HSplitTop(20.0f, &Button, &MainView);
			s_SystemScrollRegion.AddRect(Button);

			static const char *s_apEmoteNames[] = {
				"All Emotes (Cycle)",
			...
			char aBuf[128];
			str_format(aBuf, sizeof(aBuf), "Emote: %s", s_apEmoteNames[pWibeVisuals->m_ReactionSpamMode]);

			if(DoButton_CheckBox_Number(&pWibeVisuals->m_ReactionSpamMode, aBuf, pWibeVisuals->m_ReactionSpamMode, &Button))
				pWibeVisuals->m_ReactionSpamMode = (pWibeVisuals->m_ReactionSpamMode + 1) % 17;
		}
		...
		s_SystemScrollRegion.AddRect(Row);
		Row.VSplitMid(&LeftCol, &RightCol, 8.0f);
		if(DoButton_CheckBox(&pWibeVisuals->m_CheatPREnabled, "Client Promo Chat", pWibeVisuals->m_CheatPREnabled, &LeftCol))
			pWibeVisuals->m_CheatPREnabled ^= 1;
		if(DoButton_CheckBox(&pWibeVisuals->m_AutoFireEnabled, "Auto Fire", pWibeVisuals->m_AutoFireEnabled, &RightCol))
			pWibeVisuals->m_AutoFireEnabled ^= 1;
